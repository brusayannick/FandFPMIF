...
- ``module.install.registry`` — install from PyPI or npm by name. Today this
  resolves via the `flows_funds.modules` entry point (Tier 2.2). Until the
...
if TYPE_CHECKING:
    from flows_funds.api.modules.loader import ModuleLoader
...
def _install_from_registry(loader: "ModuleLoader"):
    async def handler(handle: JobHandle) -> None:
        ...
        from flows_funds.api.modules.discovery import (
            ENTRY_POINT_GROUP,
