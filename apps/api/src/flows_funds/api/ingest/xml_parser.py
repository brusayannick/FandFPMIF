...
from flows_funds.api.schemas.event_logs import XmlColumnMapping
...
def autodetect_mapping(probe: dict[str, Any]) -> XmlColumnMapping | None:
    ...
    from flows_funds.api.ingest.csv_parser import AUTODETECT_CANDIDATES, normalise_ident
...
def parse_xml(
    ...
    *,
    ...
    if effective is None:
        effective = autodetect_mapping(probe_xml(path))
        if effective is None:
            raise ValueError(
                "Could not autodetect an XML mapping. Submit an `xml_mapping` "
                "that names the event element plus case_id / activity / timestamp."
            )
