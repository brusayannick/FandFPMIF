        ...
        <CardContent className="space-y-3">
          <div className="flex flex-wrap items-start justify-between gap-2">
            <div>
              ...
              <p className="text-[11px] text-muted-foreground">
                Each run is one synthetic log generated by the same model; download any as{" "}
                {format.toUpperCase()}.
              </p>
            </div>
          ...
          <div className="flex flex-wrap gap-1.5">
            {downloads.map((d) => (
              <Button
                key={d.index}
                onClick={() => onDownload(d.index, format)}
                size="sm"
                variant="outline"
              >
...
function FormatToggle({
  ...
  const formats: ExportFormat[] = ["csv", "xes"];
  return (
    <div
      role="group"
    ...
    >
      {formats.map((f) => (
        <button
          ...
          onClick={() => onChange(f)}
          className={cn(
            "rounded px-2 py-1 text-[11px] font-medium uppercase tracking-wide transition-colors",
            value === f
